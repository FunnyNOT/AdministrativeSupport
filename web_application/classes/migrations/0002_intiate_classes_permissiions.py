...
def forwards_func(apps, schema_editor):
    ...
    my_app_config = global_apps.get_app_config('classes')
    create_contenttypes(my_app_config)

    # Can View Classes
    auth_user_content_type = ContentType.objects.get(app_label='classes', model='class')

    can_view_classes = Permission(name='Can view class', codename='can_view_class',
                                content_type=auth_user_content_type)
    can_view_classes.save()
    system_admin_group.permissions.add(can_view_classes)
    admin_user_group.permissions.add(can_view_classes)
    manager_user_group.permissions.add(can_view_classes)
    teacher_user_group.permissions.add(can_view_classes)
    student_user_group.permissions.add(can_view_classes)
